...
%Derived Parameter
dl = l/(n+1); %Distance b/w nodes
A = pi*rr^2; %Constant cross-section area
k = E*A/dl; %spring constant b/w the nodes
R = R/n;    %damping coefficient b/w the nodes
m = rho*A*l/n;
...
x0 = zeros(n,1);
%node_xpluck = randi([1 n],1);
node_xpluck = 1;
loc_xpluck = loc(node_xpluck+1);
m1 = xpluck/(loc_xpluck);
for ii = 1:node_xpluck-1
   x0(ii) = m1*dl*ii;
end
m2 = xpluck/(l-loc_xpluck);
for ii = node_xpluck:n
    x0(ii) = xpluck - m2*dl*(ii-node_xpluck);
end
